"""#24 day 3: Capon-MVDR high-resolution baseline for the crossover claim.
Per-k-bin temporal Capon: spatial 2D-FFT per frame -> time series per k-bin -> snapshot
covariance (length-M sliding subwindows, diagonal loading) -> Capon power over a fine omega
grid -> same argmax_U score as the FFT/NSP path. Super-resolves omega at short windows IF the
covariance supports it (M snapshots trade against resolution -- the honest question).
Usage: mvdr_baseline.py  (runs the crossover inventory at 8/6/4/3 s; prints the table)
"""
...
M = 10                                     # snapshot length (2 s): resolution/robustness knob
OMG = np.linspace(0.3, 12.0, 120)          # fine omega grid (FFT at 4 s gives only ~7 bins!)

def mvdr_tables(g, gsd):
    T = len(g)
    gm = g - g.mean(0)
    F2 = np.fft.fft2(gm, axes=(1, 2))                     # per-frame spatial FFT
    ...
    # subsample k-bins for speed (energy-ranked)
    E = (np.abs(F2) ** 2).sum(0).ravel()[ridx]
    keep = ridx[np.argsort(E)[::-1][:400]]
    ts = F2.reshape(T, -1)[:, keep]                       # (T, K) complex series
    nsnap = T - M + 1
    P = np.zeros((len(OMG), ts.shape[1]))
    steer = np.exp(1j * np.outer(np.arange(M) / FPS, OMG))     # (M, nOm)
    for kbin in range(ts.shape[1]):
        X = np.stack([ts[i:i + M, kbin] for i in range(nsnap)], 1)   # (M, nsnap)
        R = X @ X.conj().T / nsnap
        R += np.trace(R).real / M * 0.03 * np.eye(M)      # diagonal loading
        Ri = np.linalg.inv(R)
...
res = {d: [] for d in (8.0, 6.0, 4.0, 3.0)}
for KEY, TELG, OFF, T0S, T1S in SECTIONS:
    ...
    for T0 in np.arange(T0S, T1S - 8, 30.0):
        ...
        for DUR in (8.0, 6.0, 4.0, 3.0):
            ...
            res[DUR].append(float(np.hypot(*(U - v_r))))
        print(f"{KEY}@{T0:.0f} done", flush=True)
print("\nMVDR baseline (same windows as the crossover test):")
for DUR in (8.0, 6.0, 4.0, 3.0):
    e = np.array(res[DUR])
    if len(e):
        print(f"{DUR:4.0f}s: med {np.median(e):.2f}  p90 {np.percentile(e, 90):.2f}  n={len(e)}")
